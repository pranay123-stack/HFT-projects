...
// Define a lambda function for the custom comparator
auto orderLotsComparator = [](const Order& order) {
    return order.lots == 0;
};
...
std::vector<Trade> OrderBook::add_order(Direction direction, Order order) {
	...
	// Now use the custom comparator in your erase-remove call.

    counter_orders.erase(std::remove_if(counter_orders.begin(), counter_orders.end(), orderLotsComparator), counter_orders.end());
    // ...
	if (order.lots > 0) {
